 ...
 * Postgres Auth stores phones inconsistently (sometimes with +, sometimes without)
 */
 ...
 * an email OTP. The user is NOT created in Postgres Auth until both
...
async function signup({ email, full_name }) {
  ...
  const { data: existingUser, error: userQueryError } = await dbClient
    .from('users')
    .select('id, active')
  ...
  // Check Postgres Auth for existing user with this email (skip if inactive user found — they'll be in auth already)
  if (!existingUser || existingUser.length === 0) {
    try {
      const { data: authList, error: authListError } = await dbClient.auth.admin.listUsers({
        page: 1,
        perPage: 1000
      });
  ...
  const { error: pendingError } = await dbClient
    .from('signup_pending')
    .upsert({
...
async function sendPhoneOtpForSignup(email, phone_country_code, phone_number) {
  ...
  const { data: phoneOwners } = await dbClient
    .from('users')
    .select('email, active')
 ...
 *
 * Creates the real user in Postgres Auth + public.users with the user-chosen password.
...
async function setPasswordAndComplete(email, password) {
  ...
  const { data: publicUsers, error: publicQueryErr } = await dbClient
    .from('users')
  ...
  // 3. Check Postgres Auth (auth.users) — also save the match for reuse
  let existingAuthUser = null;
  try {
    const { data: authList } = await dbClient.auth.admin.listUsers({ page: 1, perPage: 1000 });
    if (authList?.users) {
      const authMatches = authList.users.filter(u => u.email?.toLowerCase() === normalizedEmail);
      if (authMatches.length > 1) {
        console.error('[Signup] Duplicate email in Postgres Auth:', normalizedEmail, 'count:', authMatches.length);
        return { success: false, error: 'This email is associated with multiple accounts. Please contact support.', code: 'DUPLICATE_EMAIL' };
      }
      if (authMatches.length === 1) {
    ...
    console.warn('[Signup] Postgres Auth duplicate check skipped:', authCheckErr.message);
  }
  ...
  if (existingUser) {
    ...
    // Update password (and phone if changed) in Postgres Auth
    ...
    const { error: authUpdateError } = await dbClient.auth.admin.updateUserById(existingUser.id, authUpdateData);
    if (authUpdateError) {
      console.error('[Signup] Error updating Postgres Auth user:', authUpdateError.message);
    ...
    await dbClient.from('signup_pending').delete().eq('email', normalizedEmail);
    await dbClient.from('signup_otps').delete().eq('identifier', normalizedEmail);
    await dbClient.from('signup_otps').delete().eq('identifier', fullPhone);
  ...
  // --- CREATE OR UPDATE PATH based on Postgres Auth state ---
  let dbUser;
  ...
  if (existingAuthUser) {
    ...
    try {
      ...
      if (phoneOwner) {
        ...
        await dbClient.auth.admin.updateUserById(phoneOwner.id, { phone: '+10000000000' });
      }
    } catch (phoneCheckErr) {
      console.warn('[Signup] Phone conflict check skipped:', phoneCheckErr.message);
    }

    const { error: authUpdateErr } = await dbClient.auth.admin.updateUserById(existingAuthUser.id, {
      password,
      phone: fullPhone,
      phone_confirm: true,
      user_metadata: {
    ...
    let { data: authData, error: authError } = await dbClient.auth.admin.createUser({
      email: normalizedEmail,
      password,
      email_confirm: true,
      phone: fullPhone,
    ...
    if (authError && authError.message?.includes('Phone number already registered')) {
      ...
      try {
        ...
        if (phoneOwner) {
          ...
          const retry = await dbClient.auth.admin.createUser({
            email: normalizedEmail,
            password,
            email_confirm: true,
            phone: fullPhone,
