...
async function validateColumns(table, columns) {
  ...
  const { error } = await serverDb.rpc("_ai_validate_columns", {
    p_table: table,
    p_columns: cleaned,
  });
  if (!error) return;
...
export async function executeTableQuery(
  ...
  // PostgREST/db-js has no native column-vs-column filter syntax. When
  ...
  let query = serverDb.from(params.table).select(params.select || "*");
...
async function executeRowsViaRpc(
  ...
  const { data, error } = await serverDb.rpc("ai_select_rows", {
...
export async function executeAggregate(
  ...
  const { data, error } = await serverDb.rpc("ai_aggregate", {
    p_table: params.table,
    p_filters: filtersToJsonb(params.filters),
...
export async function executeCount(params) {
  ...
  const { data, error } = await serverDb.rpc("ai_count", {
    p_table: params.table,
    p_filters: filtersToJsonb(params.filters),
  });
